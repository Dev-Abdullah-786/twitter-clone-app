...
export default function PostButtons({
  ...
  const [likesCount, setLikesCount] = useState<number>(likesCountDefault);
  const [likedByMe, setLikedByMe] = useState<boolean>(likedByMeDefault);

  async function toggleLike() {
    try {
      const response = await axios.post<{ like?: boolean }>("/api/like", {
        id,
      });
  ...
  const likeColor = likedByMe ? "#ef4444" : "#71767b";
